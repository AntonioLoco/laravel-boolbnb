...
@section('content')
    <div class="container mt-3">
        <a href="{{ route('admin.apartments.index', $apartment->slug) }}" class="btn btn-secondary">
        ...
        <form action="{{ route('admin.apartments.update', $apartment->slug) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-2 position-relative">
                ...
                <input type="text" class="form-control @error('street_address') is-invalid @enderror" id="street_address"
                    name="street_address" value="{{ old('street_address', $apartment->address->street_address) }}">
            ...
            <div class="mb-2 position-relative">
                ...
                <input type="number" class="form-control @error('house_number') is-invalid @enderror" id="house_number"
                    name="house_number" value="{{ old('house_number', $apartment->address->house_number) }}">
                @error('house_number')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-2 position-relative">
                ...
                <input type="number" class="form-control @error('postal_code') is-invalid @enderror" id="postal_code"
                    name="postal_code" value="{{ old('postal_code', $apartment->address->postal_code) }}">
                @error('postal_code')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                @foreach ($services as $service)
                    <div class="form-check">
                        <input class="form-check-input check-service @error('services') is-invalid @enderror"
                            ...
                            name="services[]" @checked($errors->any() ? in_array($service->id, old('services', [])) : $apartment->services->contains($service))>
